 ...
 * Format: codesLength (1 byte) + utf8-encoded builder code + schemaId (1 byte) + ERC marker (16 bytes)
 *
...
export function encodeBuilderCode(builderCode: string): Hex {
  ...
  return `0x${codeLengthHex}${codeHex}${ERC8021_SCHEMA_ID}${ERC8021_MARKER}` as Hex;
}
