...
export interface paths {
    ...
    "/v1/tvl/timeseries": {
         ...
         *
         *     A numeraire token with no USD price for part of the window is named in `meta.partial_errors`, and the balances it backs are excluded from the totals on those days. `TOKEN_PRICE_UNAVAILABLE` is a gap that should close, so it is retried on the next read; `TOKEN_PRICE_UNAVAILABLE_ACCEPTED` is one that cannot, with the reason in its message. Both mean the totals are short by that token; only the first is worth alerting on.
         */
    ...
    "/v1/vaults/slug/{slug}": {
         ...
         * @description Returns every indexed deployment associated with one exact Admin vault slug. The lookup serves visible and hidden enabled vaults without making hidden vaults enumerable; disabled and unknown slugs return 404. Each item has the same shape as an item from `GET /v1/vaults`.
         */
        get: operations["get_vaults_by_slug"];
        put?: never;
        post?: never;
        delete?: never;
        options?: never;
